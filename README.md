...
- Editable invoices, independent of the quote (confirmed with owner 2026-08-11; the real driver of this round of changes). Today the invoice's Scope of work section is read-only and re-derived from the quote's `calculation_data.clientFacingLines` every time it renders, so the invoice is NOT independent of the quote. Chad wants: (a) adder comment boxes on BOTH the quote and the invoice (the quote already has them; the invoice's scope comments must become editable), and (b) the invoice editable independently of the original quote — the quote stays the point-in-time estimate and final adjustments happen on the invoice, not by re-editing the quote. Decided design: snapshot the quote's line items + comments into `invoice_data.scopeLines` when invoicing is first set up (the invoice is seeded from the quote once, then lives independently), and make those scope lines (name + comment) editable on the invoice; keep the charge amounts on the existing setup form (contract amount / rough-in % / finish % / permit fee), re-editable after first save — the invoice scope lines carry comments only, NOT per-line amounts (amounts stay on the percentage model; decided 2026-08-11). The quote is NOT hard-locked (point-in-time is a workflow rule). A paid invoice is admin-overrideable with a warning, and the payment record is preserved — this softens the earlier "lock paid rough-in" item below (nothing is truly immutable; the lock is an admin-gated warning, not a hard block). Build touches: `invoice_data` shape (new `scopeLines`), the invoice setup form / a new edit-invoice page, `lib/invoice-pdf.ts`, `components/pdf/invoice-document.tsx`, the invoice print page, the `/receivables` view, and the dashboard. Existing invoices (already set up) should backfill `scopeLines` from their quote's `calculation_data.clientFacingLines` on first edit so nothing breaks.
