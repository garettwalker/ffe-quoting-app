...
- Lock paid rough-in when the quote changes after rough-in payment (owner to confirm). Realistic flow: a quote is an initial estimate; after rough-in the scope changes, so the quote gets a final edit to the real final total. If the customer has already paid the rough-in (initial) invoice, that paid amount should be locked and only the finish invoice should recompute from the new final total (finish = new contract total minus paid rough-in), so the customer is never re-billed for a paid invoice. **Current behavior:** `invoice_data` is an independent JSONB snapshot (contract amount, split %, permit fee, two invoice records with stored amounts/status/paidAt); editing a quote after invoice setup does NOT auto-update the invoices, and there is no lock-paid/recompute-finish logic today — the owner manually adjusts invoice setup. Open design questions: does the finish recompute on any accepted-quote edit or only an explicit "finalize after rough-in" action; does the owner-set split hold or is the finish overridden to the residual so the two always net to the new total; what "new contract total" is used (edited `clientQuoteTotalCents` vs an owner-entered final contract); permit-fee handling. Pairs with the project status tracker.
