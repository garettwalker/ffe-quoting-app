...
4. In the Stripe dashboard (test mode): **Developers → Webhooks → Add endpoint** pointing at `https://admin.freedomfamilyelectric.com/api/stripe-webhook`, listening for `checkout.session.completed`, `payment_intent.succeeded`, `payment_intent.processing`, `payment_intent.payment_failed`, and `charge.refunded`. Stripe shows the **signing secret** — set it as `STRIPE_WEBHOOK_SECRET` in Vercel and redeploy.
