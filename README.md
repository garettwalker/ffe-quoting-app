...
**Endpoint is now gated (Phase B).** `/api/email-pdf` requires an admin session — anonymous callers get 401, non-admins 403 (`getServerUser()` in `lib/auth.ts`). The middleware front-door redirect keeps anonymous users off the pages that expose the button. Note: RLS is still permissive `anon` until Phase C, so the database itself is not yet private — that's the remaining hardening step.
...
Login is being added in four low-risk phases so the live app keeps working between each. Phases A + B are **live**: you must sign in to use the app, and the email endpoint is gated to admins. RLS is still permissive `anon` (Phase C is the remaining hardening — the database itself is not yet private).
...
- **Phase A (done):** auth infrastructure + login + logout. App stays fully open; login works but is optional.
- **Phase B (done):** middleware redirects unauthenticated users to `/login` (except public routes: `/login`, `/pay/*`, `/api/keepalive`, `/api/stripe-webhook`); `POST /api/email-pdf` is gated to an admin session (401/403). The app now requires sign-in; RLS is still permissive anon.
- **Phase C (next):** tighten RLS role-aware. For each table: add `authenticated`/`admin` policies alongside the existing `anon` ones, switch the app's data access from the anon client to the authenticated server + browser clients, verify logged-in reads/writes work, then drop the `anon` policies. Switch the keepalive route to a service-role client (it has no user session). Critical gotcha: an authenticated user is *not* `anon`, so the new policies must exist before the clients switch (or every read 401s). Requires a new `SUPABASE_SERVICE_ROLE_KEY` env var. Rollback: re-add `anon` policies via the SQL Editor.
...
- Owner/admin login (Supabase Auth), one owner + one builder/admin. This also gates the currently-open `POST /api/email-pdf` send endpoint (today anyone with the URL can send as the business; tightening is the documented next step once the email feature is verified working).
