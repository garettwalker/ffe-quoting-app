...
Login is being added in four low-risk phases so the live app keeps working between each. Phases A, B, C1, and C2 are **live**: you must sign in to use the app, the email endpoint is gated to admins, and all data access goes through the logged-in session (the authenticated server + browser clients). The legacy wide-open `anon` policies still exist, so the database is not yet private — C3 drops them (the final hardening step).
...
- **Phase C3 (next):** drop the legacy `anon` policies (and revoke the anon grant on `next_quote_id`). After this the database is fully private — anonymous direct access is denied. SQL: `supabase/phase-c-3-drop-anon-policies.sql`. Rollback: re-add the `anon` policies via the SQL Editor.
...
- Owner/admin login (Supabase Auth), one owner + one builder/admin. **In progress** — Phases A, B, C1, C2 are live (sign-in required, email endpoint gated to admins, all data access through the logged-in session). Remaining: C3 drops the legacy anon policies (database fully private), D confirms role gates. The `POST /api/email-pdf` send endpoint is already gated to admins (Phase B).
...
- Tighten Supabase RLS for production (remove anon policies, add auth) — includes the five new pricing tables and `app_settings`; gate `/pricing-admin` to admin. **In progress** — Phase C1 added `authenticated`/`admin` policies (via `public.is_admin()`) and C2 switched all data access to the logged-in session (server + browser clients; keepalive on the service-role key). Remaining: Phase C3 drops the legacy `anon` policies so the database is fully private. `/pricing-admin` is effectively admin-only already (its tables are admin-only under the new policies; a team member would see an empty page).
