...
- Project status tracker (designed, not built). A dedicated Projects view that follows each accepted job through its field lifecycle in a pizza-delivery-style stage strip: Quote, Rough-in, Rough-in Inspection, Finish, Final Inspection, plus the existing billing/invoicing steps (Pending Payments, Paid in Full). Scope decided 2026-08-11: a new Projects view separate from the `/quotes` pipeline (the pipeline keeps the quote lifecycle); job costing (payroll + wire/material expenses per job, margin) is a later phase, not v1. Two HTML mockups (collapsible right-side nav + tracker board) were produced for review; build waits on approval. Open design questions: where stage state lives (a new `project_status` column/JSONB on `quotes`, or a separate `projects` table), whether field stages are manual status buttons or crew-assignment-driven (ties into the existing scheduling tool), and how the stage strip maps onto the existing derived lifecycle so the two views never disagree.
