...
- **Editable Scope of Work (independent of the quote).** The invoice setup form also has a **Scope of Work** editor: a list of line items, each with a name and a customer-facing comment, that prints in a Scope of Work section on both invoices (Initial + Finish, on screen + PDF). The list is **seeded from the quote** (its line items + comments) the first time invoicing is set up, then lives on the invoice in `invoice_data.scopeLines` and is edited there independently of the quote — the quote stays a point-in-time estimate and final adjustments happen on the invoice. Scope lines carry **names + comments only** (no per-line amounts; amounts stay on the contract / split above). Editing scope names/comments on a paid invoice is safe and does not touch any money; only changing the contract / split / permit amounts can reset a paid invoice (see below).

Invoice setup is stored as JSONB in the `quotes.invoice_data` column (null until set up). Saving invoice setup preserves existing paid statuses by invoice kind — **except** that if the owner's current contract/split/permit inputs would change the amount of an already-paid invoice, saving resets that invoice to unpaid (with a clear warning beforehand) so the owner re-marks it paid at the new amount; a paid invoice's amount is never changed silently. `invoice_data.scopeLines` is optional so invoices set up before the field existed still load: the PDF/print path backfills the scope from the quote's `calculation_data.clientFacingLines` when `scopeLines` is absent, and the first time the owner re-saves the setup the builder persists `scopeLines` so the invoice becomes independent from that save on. Invoicing math lives in `lib/invoice-calculations.ts`.
...
- Invoicing from accepted quotes: contract amount, 50/50 rough-in/finish split (editable), permit fee, two invoices (initial = rough-in + permit, finish = remainder), paid/unpaid tracking, printable invoices, outstanding balance on the dashboard. The invoice setup form also has an **editable Scope of Work** editor (line names + comments) seeded from the quote once then independent on the invoice (`invoice_data.scopeLines`), so the quote stays the point-in-time estimate and final adjustments happen on the invoice.
...
- ~~Editable invoices, independent of the quote~~ **Done (2026-08-11).** The invoice setup form now has an editable **Scope of Work** editor (line names + customer-facing comments) seeded from the quote once, then saved on the invoice in `invoice_data.scopeLines` and edited independently of the quote. Scope lines carry comments only (no per-line amounts; amounts stay on the contract / split). The quote stays the point-in-time estimate; final adjustments happen on the invoice. A paid invoice's amount is never changed silently: changing the contract/split/permit amounts resets a paid invoice to unpaid with a warning, but editing scope comments is money-free and never resets anything. Existing invoices backfill scope from the quote's `calculation_data.clientFacingLines` until the owner re-saves (which persists `scopeLines`). This supersedes the old "lock paid rough-in" item below.
...
- 2026-08-11: Editable invoice Scope of Work, independent of the quote (confirmed with Chad). The invoice setup form (`components/invoice-builder.tsx`) now has an editable **Scope of Work** section: a list of line items, each with a name and a customer-facing comment, with add / remove / edit. The list is seeded from the accepted quote (its `calculation_data.clientFacingLines` — names + comments) the first time invoicing is set up, then saved on the invoice in a new `invoice_data.scopeLines` field and edited there independently of the quote. The quote stays the point-in-time estimate; final adjustments (including editing which lines and their comments) happen on the invoice. Scope lines carry **names + comments only** — no per-line amounts (amounts stay on the contract / rough-in % / finish % / permit form, decided 2026-08-11). The Scope of Work section prints on both invoices (Initial + Finish, on screen + PDF): `lib/invoice-pdf.ts` now reads `scopeLines` from `invoice_data` first and backfills from `calculation_data.clientFacingLines` only when the field is absent (so old invoices keep working and become independent on the owner's next save); the on-screen invoice print page and `components/pdf/invoice-document.tsx` render it. Editing scope names/comments is money-free, so it never resets a paid invoice; only changing contract/split/permit amounts can (existing warning + reset-to-unpaid logic, preserved). New `scopeLines?` on `InvoiceData` in `lib/types.ts` (optional, no migration). `app/quotes/[id]/invoices/page.tsx` passes the quote's clientFacingLines as `seedScopeLines`. No em dashes in copy. Verified: `tsc --noEmit` green.
