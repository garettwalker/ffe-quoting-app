...
5. Test end-to-end: from the admin invoicing page, copy a payment link, open it, pay with the test card `4242 4242 4242 4242` (any future date + any CVC) and confirm the invoice flips to paid + a `payments` row appears (`method = card`, `recorded_by = stripe`). Then test an ACH/bank payment (test banks are in Stripe's docs) and confirm it lands as `processing` then `succeeded` a moment later. Watch the endpoint's event log in the Stripe dashboard to confirm events are received 200.
