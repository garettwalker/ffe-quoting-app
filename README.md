...
- 2026-08-11: Invoice line-item table UX: `$` prefix on Unit Price, `qty` label next to the Quantity input, and a save-time price-change confirmation. The owner accidentally edited a unit price instead of a quantity, so the Unit Price input now shows a leading `$` (clearly a dollar field, distinct from the quantity number) and the Quantity input has a `qty` label beside it. As a backstop, on Save the builder lists any line loaded from the saved invoice whose unit price changed and asks for confirmation (`window.confirm`); Cancel aborts the save so the owner can review. Newly added lines are excluded (setting their price is expected). All in `components/invoice-builder.tsx`. Verified: `tsc --noEmit` green.
