...
lib
  ...
  supabase-admin.ts             // server-only service-role client (bypasses RLS; keepalive + future Stripe webhook; never reaches the browser)
...
Flow: a Pay button on `/pay/[token]` posts the token to `/api/create-checkout-session`, which re-reads the amount from the DB and (once Stripe is wired) returns a Stripe Checkout URL to redirect to. Stripe redirects back to `/pay/success` or `/pay/canceled`, but the **signed webhook** (`/api/stripe-webhook`) is the source of truth for confirming payment — the redirect URL can be tampered with, so the webhook is what writes the `payments` ledger row and flips the invoice's paid flag. The webhook is signature-verified and idempotent (a `stripe_webhook_events` log keyed by Stripe event id, to be added in step 3, so a replayed/retried event applies exactly once). ACH (US bank account) payments settle with a delay and can be returned/refunded; step 3 handles the `payment_intent.succeeded`/`charge.refunded`/return events and a per-transaction cap to confirm during testing.
...
- **Admin invoicing page:** each invoice card shows a **Copy payment link** button (server-mints the URL, client copies it + a Preview link) whenever `PAY_LINK_SECRET` + `APP_URL` are set, so the owner can test the `/pay` page even before Stripe is wired.

**Status:** step 2 (this pass) built the full plumbing with **no Stripe keys or package required** — `lib/pay-token.ts`, the public `/pay/[token]` + `/pay/success` + `/pay/canceled` pages, the `/api/create-checkout-session` + `/api/stripe-webhook` route skeletons (both return a clear "not configured" until their keys are set), the email pay-link append (gated), and the admin copy-link button. Step 3 wires the real Stripe Checkout Session (card + ACH) and the live webhook handler; that needs `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY`, registering the webhook in the Stripe dashboard (test mode), and enabling the US bank account payment method. No card numbers, CVC, or full bank details are ever stored — Stripe holds them.
