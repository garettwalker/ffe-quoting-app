...
- Site nav re-layout (designed, not built). The top menu is getting crowded as tools are added (overview, quotes, receivables, schedule, pricing admin, email log, login). Mockup proposes a collapsible right-side nav that lists every tool and collapses to an icon rail to keep the header clean. Build waits on approval of the mockup.
