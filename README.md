...
- Lock paid rough-in when the quote changes after rough-in payment (owner to confirm) — **superseded by the "Editable invoices" item above.** The owner's confirmed direction is that final adjustments happen by editing the invoice directly (independently of the quote), not by re-editing the quote. A paid invoice is admin-overrideable with a warning (not a hard lock), and the payment record is preserved. Kept here for context; the live design is the Editable invoices item.
