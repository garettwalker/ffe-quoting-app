...
3. Add `RESEND_API_KEY` and `EMAIL_FROM` (an address on the verified domain) to Vercel env and `.env.local`.
...
EMAIL_FROM                         // RFC "From" address on a Resend-verified domain,
                                   //   e.g. "Freedom Family Electric <quotes@yourdomain.com>"
```
The Supabase URL is the base project URL, **not** the REST endpoint. The two `RESEND_*`/`EMAIL_FROM` vars are server-only (no `NEXT_PUBLIC_` prefix) and are only needed once the email feature is enabled. See **Sending email from the app** below.
